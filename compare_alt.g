...
comp_alt := function(n)
    local i, G, summe, E1, E2, oG1, oG2, ergebnisse, file_res;
...
#produce a file we can analyze with python to plot etc. with n and the values 
    ...
    file_res := OutputTextFile("results_compare_alt.txt", false); 
    WriteLine(file_res, "GroupName, GroupOrder, E(G)o(G)value, E(G)o(G)value_decimal");
...
#we do the same again, but this time o(G))
    ...
    for i in [3..n] do
        ...
        Print("n = ", i, " → o(G) =", oG1, " ", oG2, "\n");
        WriteLine(file_res, Concatenation("AoG", String(i), ",", String(Order(G)), ",", String(oG1), ",", String(oG2)));
        Add(ergebnisse, E1);
    od;    
