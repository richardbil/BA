...
comp_cpi := function(m, n)
    local primes, p, i, G, g, summe, E1, E2, oG1, oG2, ergebnisse, file_res;

    # initialize an empty list to store the results
    ergebnisse := [];

    # produce a file we can analyze with Python
    file_res := OutputTextFile("results_compare_cpi.txt", false);
    ...
    for p in primes do
        for i in [1..n] do
            ...
            Print("C_", p^i, " → E(G) = ", E1, " ", E2, " ", oG1, " ", oG2, "\n");
            WriteLine(file_res, Concatenation("C_", String(p^i), ",", String(Order(G)), ",", String(E1), ",", String(E2), ",", String(oG1), ",", String(oG2)));
            Add(ergebnisse, E1);
        od;
    od;
